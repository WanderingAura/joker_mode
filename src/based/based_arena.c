...
{ 
  ...
  if(base == 0)
  {
    ...
    {
      reserve_size = AlignPow2(reserve_size, vos_GetSystemInfo()->large_page_size);
      commit_size  = AlignPow2(commit_size,  vos_GetSystemInfo()->large_page_size);
    }
    else
    {
      reserve_size = AlignPow2(reserve_size, vos_GetSystemInfo()->page_size);
      commit_size  = AlignPow2(commit_size,  vos_GetSystemInfo()->page_size);
    }
    
    if(params->flags & ArenaFlag_LargePages)
    {
      base = vos_ReserveMemoryLarge(reserve_size);
      vos_CommitMemoryLarge(base, commit_size);
    }
    else
    {
      base = vos_ReserveMemory(reserve_size);
      vos_CommitMemory(base, commit_size);
    }
...
arena_push(Arena *arena, U64 size, U64 align, B32 zero)
{
  ...
  if(current->cmt < pos_pst)
  {
    ...
    {
      vos_CommitMemoryLarge(cmt_ptr, cmt_size);
    }
    else
    {
      vos_CommitMemory(cmt_ptr, cmt_size);
    }
...
arena_pop_to(Arena *arena, U64 pos)
{
  ...
  {
    prev = current->prev;
    vos_ReleaseMemory(current, current->res);
  }
#endif
